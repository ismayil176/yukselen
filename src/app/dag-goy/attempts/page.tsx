      ...
      attempts: (prev?.attempts ?? []).map((item) => (
        item.id === id
          ? {
              ...
              seenAtAz: seen ? formatAzDateTimeUi(new Date()) : undefined,
            }
          : item
      )),
    }));
                  ...
                  return (
                    <div key={a.id} className={`rounded-2xl border p-4 ${checked ? "border-purple-300 bg-purple-50/60" : "border-black/10 bg-slate-50"}`}>
                      ...
                      <div className="mt-3 grid gap-2 text-sm text-slate-700">
                        ...
                        <div><span className="font-medium">Başladı:</span> {a.startedAtAz ?? a.startedAt}</div>
                        <div><span className="font-medium">Bitdi:</span> {a.finishedAtAz ?? a.finishedAt ?? "—"}</div>
                      </div>
                    </div>
                  );
                })}
                    ...
                    {attempts.map((a) => {
                      ...
                      return (
                        <tr key={a.id} className={`border-t align-top ${checked ? "border-purple-200 bg-purple-50/60" : "border-black/10"}`}>
                          ...
                          <td className="py-3 pr-3">{a.startedAtAz ?? a.startedAt}</td>
                          <td className="py-3">{a.finishedAtAz ?? a.finishedAt ?? "—"}</td>
                        </tr>
                      );
                    })}
                  </tbody>
