...
import { removeBrowserValue, writeBrowserJson, writeBrowserValue } from "@/lib/browserExamStorage";
...
export function CandidateClient({ category, examId, examTitle }: { category: string; examId: string; examTitle: string }) {
  ...
  const progressKey = useMemo(() => `progress:${category}:${examId}`, [category, examId]);
  ...
  async function startExam() {
    ...
    const normalizedPhone = normalizeAzerbaijanPhone(form.phone);
    const payload = {
    ...
    if (!payload.firstName || !payload.lastName || !payload.fatherName || !form.phone.trim()) {
    ...
    try {
      ...
      removeBrowserValue(resultKey);
      writeBrowserJson(storageKey, payload);
      writeBrowserValue(attemptKey, String(j.attemptId));
      ...
      <Container>
        <div className="mx-auto max-w-xl rounded-2xl border border-black/10 bg-white p-6 sm:p-8">
          ...
          <div className="mt-6 grid gap-4">
            ...
            <Field label="Nömrə" value={form.phone} onChange={(v) => setForm((p) => ({ ...p, phone: v }))} placeholder="055 555 55 55 və ya +99455 555 55 55" inputMode="tel" autoComplete="tel" />
          </div>
