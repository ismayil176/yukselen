...
export async function POST(req: Request) {
  ...
  try {
    ...
    const normalizedPhone = normalizeAzerbaijanPhone(parsed.data.student.phone);
    if (!normalizedPhone) {
      return noStoreJson({ ok: false, error: "Telefon nömrəsini düzgün daxil edin." }, { status: 400 });
    }
    ...
    const attempt = await createAttempt({ ...parsed.data, student: { ...parsed.data.student, phone: normalizedPhone } });
